...
import { ApiLawyerCard } from "./ApiLawyerCard";

interface ApiLawyerCardGridProps {
  lawyers: ApiLawyer[];
...
export function ApiLawyerCardGrid({
  lawyers,
  variant = "default",
  ...
  return (
    <div className={getGridClasses()}>
      {lawyers.map(lawyer => (
        <ApiLawyerCard key={lawyer.id} lawyer={lawyer} variant={variant} showFullDetails={showFullDetails} />
      ))}
    </div>
  );
}
