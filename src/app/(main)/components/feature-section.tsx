          ...
          <div
            ...
            {PROCESS_STEPS.slice(rowIndex * 3, (rowIndex + 1) * 3).map((processStep, index) => (
              <div
                ...
                <CardProcessStep processStep={processStep} index={rowIndex * 3 + index} />
              </div>
            ))}
          </div>
        ))}
          ...
          <div
            ...
            {PROCESS_STEPS.slice(Math.floor(PROCESS_STEPS.length / 3) * 3).map((processStep, index) => (
              <div
                ...
                data-aos="zoom-in-up"
                data-aos-duration="600"
                data-aos-delay={300 + Math.floor(PROCESS_STEPS.length / 3) * 100 + index * 50}>
                <CardProcessStep processStep={processStep} index={Math.floor(PROCESS_STEPS.length / 3) * 3 + index} />
              </div>
            ))}
          </div>
        )}
